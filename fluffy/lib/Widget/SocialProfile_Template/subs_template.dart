...
class Subs_TMP extends StatelessWidget {
  const Subs_TMP({Key key, this.itemtext, this.nameuser, this.userimage})
      : super(key: key);
  final Text itemtext;
  final Text nameuser;
  ...
  Widget build_social() {
    if (itemtext.data == "Facebook") {
      ...
      return Example_data().build_subs_insta();
    } else if (itemtext.data == "Twitter") {
      return Example_data().build_subs_twitter();
    } else if (itemtext.data == "LinkedIn") {
      return Example_data().build_subs_linkedin();
    } else
      return Container();
  }
